...
from chapkit.core.exceptions import (
    BadRequestError,
    ChapkitException,
    ConflictError,
    ErrorType,
...
def test_chapkit_exception_with_defaults() -> None:
    """Test ChapkitException with default values."""
    exc = ChapkitException("Something went wrong")
...
def test_chapkit_exception_with_custom_values() -> None:
    """Test ChapkitException with custom values."""
    exc = ChapkitException(
        "Custom error",
...
def test_error_type_constants() -> None:
    ...
    assert ErrorType.NOT_FOUND == "urn:chapkit:error:not-found"
    assert ErrorType.VALIDATION_FAILED == "urn:chapkit:error:validation-failed"
    assert ErrorType.CONFLICT == "urn:chapkit:error:conflict"
    assert ErrorType.INVALID_ULID == "urn:chapkit:error:invalid-ulid"
    assert ErrorType.INTERNAL_ERROR == "urn:chapkit:error:internal"
    assert ErrorType.UNAUTHORIZED == "urn:chapkit:error:unauthorized"
    assert ErrorType.FORBIDDEN == "urn:chapkit:error:forbidden"
    assert ErrorType.BAD_REQUEST == "urn:chapkit:error:bad-request"
